...
final class InstallerSupport {
    ...
    static InstallationPlan defaultPlan(CommandLine cli) {
        ...
        Path installDir = normalize(cli.installDir(), Path.of(System.getProperty("user.home")).resolve("daisybase"));
    ...
    static InstallationPlan promptForPlan(InstallationPlan defaults, Reader input, Writer output) throws IOException {
        ...
        output.write("DaisyBase installation wizard\n");
        output.write("==========================\n");
        output.flush();
    ...
    static void performInstall(InstallationPlan plan) throws IOException {
        ...
        Files.writeString(configDir.resolve("daisybase.properties"), renderConfig(plan));
        Files.writeString(configDir.resolve("javadb.properties"), renderConfig(plan));
        writeLauncher(plan.installDir().resolve("daisybase-server.bat"), renderWindowsLauncher("server", plan));
        writeLauncher(plan.installDir().resolve("daisybase-cli.bat"), renderWindowsLauncher("cli", plan));
        ...
        writeLauncher(plan.installDir().resolve("daisybase-server.sh"), renderShellLauncher("server", plan));
        writeLauncher(plan.installDir().resolve("daisybase-cli.sh"), renderShellLauncher("cli", plan));
    ...
    private static String renderWindowsLauncher(String kind, InstallationPlan plan) {
        ...
        builder.append("set \"DAISYBASE_HOME=%~dp0\"\n");
        builder.append("set \"JAVADB_HOME=%DAISYBASE_HOME%\"\n");
        if (plan.javaHome() != null) {
            builder.append("set \"JAVA_HOME=").append(plan.javaHome()).append("\"\n");
        }
        builder.append("set \"DAISYBASE_CONFIG=%DAISYBASE_HOME%config\\daisybase.properties\"\n");
        builder.append("if not exist \"%DAISYBASE_CONFIG%\" set \"DAISYBASE_CONFIG=%DAISYBASE_HOME%config\\javadb.properties\"\n");
        builder.append("call \"%DAISYBASE_HOME%").append(command).append("\\bin\\").append(command)
                .append(".bat\" --config \"%DAISYBASE_CONFIG%\"\n");
    ...
    private static String renderShellLauncher(String kind, InstallationPlan plan) {
        ...
        builder.append("DAISYBASE_HOME=\"$(cd \"$(dirname \"$0\")\" && pwd)\"\n");
        builder.append("JAVADB_HOME=\"$DAISYBASE_HOME\"\n");
        if (plan.javaHome() != null) {
            builder.append("export JAVA_HOME=\"").append(plan.javaHome()).append("\"\n");
        }
        builder.append("DAISYBASE_CONFIG=\"$DAISYBASE_HOME/config/daisybase.properties\"\n");
        builder.append("if [ ! -f \"$DAISYBASE_CONFIG\" ]; then\n");
        builder.append("  DAISYBASE_CONFIG=\"$DAISYBASE_HOME/config/javadb.properties\"\n");
        builder.append("fi\n");
        builder.append("exec \"$DAISYBASE_HOME/").append(command).append("/bin/").append(command)
                .append("\" --config \"$DAISYBASE_CONFIG\"\n");
        return builder.toString();
    }

    private static String renderReadme(InstallationPlan plan) {
        return """
                DaisyBase installation complete.

                Launchers:
                  - daisybase-server.bat / daisybase-server.sh
                  - daisybase-cli.bat / daisybase-cli.sh
                  - start-server.bat / start-server.sh
                  - run-cli.bat / run-cli.sh

                Editable configuration:
                  config/daisybase.properties
                  config/javadb.properties (legacy compatibility alias)
