...
final class DaisyBaseJdbcObjects {
    ...
    static Clob toClob(String text) {
        return text == null ? null : new DaisyBaseClob(text);
    }

    static NClob toNClob(String text) {
        return text == null ? null : new DaisyBaseNClob(text);
    }
    ...
    static Blob toBlob(String text) throws SQLException {
        ...
        DaisyBaseBlob blob = new DaisyBaseBlob();
    ...
    static Array toArray(String text) throws SQLException {
        ...
        if (!text.startsWith(ARRAY_PREFIX)) {
            throw new SQLFeatureNotSupportedException("Array accessors require values produced by DaisyBase array wrappers");
        }
    ...
    static Array toArray(Common.Value value) throws SQLException {
        ...
        return switch (value.type()) {
            case ARRAY -> {
                ...
                yield new DaisyBaseArray(arrayValue.baseTypeName(), elements);
            }
    ...
    static Struct toStruct(String text) throws SQLException {
        ...
        if (!text.startsWith(STRUCT_PREFIX)) {
            throw new SQLFeatureNotSupportedException("Struct accessors require values produced by DaisyBase struct wrappers");
        }
    ...
    static Struct toStruct(Common.Value value) throws SQLException {
        ...
        return switch (value.type()) {
            case STRUCT -> {
                ...
                yield new DaisyBaseStruct(structValue.sqlTypeName(), attributes);
            }
    ...
    static Ref toRef(String text) throws SQLException {
        ...
        if (!text.startsWith(REF_PREFIX)) {
            throw new SQLFeatureNotSupportedException("Ref accessors require values produced by DaisyBase ref wrappers");
        }
        String[] parts = text.substring(REF_PREFIX.length()).split("\\|", -1);
        if (parts.length != 2) {
            throw new SQLException("Corrupt ref payload");
        }
        return new DaisyBaseRef(decodeText(parts[0]), decodeScalar(parts[1]));
    }
    ...
    static Ref toRef(Common.Value value) throws SQLException {
        ...
        return switch (value.type()) {
            case REF -> new DaisyBaseRef(value.asRef().baseTypeName(), toJdbcValue(value.asRef().referencedValue()));
    ...
    static RowId toRowId(String text) throws SQLException {
        ...
        if (!text.startsWith(ROWID_PREFIX)) {
            throw new SQLFeatureNotSupportedException("RowId accessors require values produced by DaisyBase row-id wrappers");
        }
        return new DaisyBaseRowId(Base64.getDecoder().decode(text.substring(ROWID_PREFIX.length())));
    }
    ...
    static RowId toRowId(Common.Value value) throws SQLException {
        ...
        return switch (value.type()) {
            case ROWID -> new DaisyBaseRowId(value.asRowIdBytes());
    ...
    static SQLXML toSqlXml(String text) throws SQLException {
        ...
        DaisyBaseSqlXml sqlxml = new DaisyBaseSqlXml();
