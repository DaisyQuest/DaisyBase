...
final class DaisyBaseResultSets {
    ...
    static ResultSet fromStatementResult(EngineApi.StatementResult statementResult, long maxRows,
        ...
        if (updatableDescriptor != null) {
            return DaisyBaseUpdatableResultSet.create(statement, rowSet, updatableDescriptor);
        }
    ...
    private static void updateRowSet(CachedRowSet rowSet, int columnIndex, Common.Value value) throws SQLException {
        ...
        switch (value.type()) {
            ...
            case BLOB -> rowSet.updateObject(columnIndex, DaisyBaseJdbcObjects.toBlob(value));
            case ARRAY -> rowSet.updateObject(columnIndex, DaisyBaseJdbcObjects.toArray(value));
            case STRUCT -> rowSet.updateObject(columnIndex, DaisyBaseJdbcObjects.toStruct(value));
            case REF -> rowSet.updateObject(columnIndex, DaisyBaseJdbcObjects.toRef(value));
            case ROWID -> rowSet.updateObject(columnIndex, DaisyBaseJdbcObjects.toRowId(value));
            case SQLXML -> rowSet.updateObject(columnIndex, DaisyBaseJdbcObjects.toSqlXml(value));
    ...
    private static ResultSet readOnlyResultSet(CachedRowSet rowSet, int resultSetType) {
        return (ResultSet) Proxy.newProxyInstance(
                DaisyBaseResultSets.class.getClassLoader(),
