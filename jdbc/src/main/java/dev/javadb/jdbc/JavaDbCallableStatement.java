...
final class JavaDbCallableStatement extends JavaDbPreparedStatement implements CallableStatement {
    private final JavaDbCallableSql callableSql;
    ...
    JavaDbCallableStatement(JavaDbConnection connection, int resultSetType, int resultSetConcurrency,
                            String sql) throws SQLException {
        this(connection, resultSetType, resultSetConcurrency, JavaDbCallableSql.parse(sql));
    }

    private JavaDbCallableStatement(JavaDbConnection connection, int resultSetType, int resultSetConcurrency,
                                    JavaDbCallableSql callableSql) throws SQLException {
    ...
    public ParameterMetaData getParameterMetaData() {
        return JavaDbParameterMetaData.create(signature.parameterDescriptors());
    }

    @Override
    protected String renderedSql() throws SQLException {
        List<JavaDbPreparedSql.BoundParameter> parameters = new ArrayList<>(parameterValues());
        for (CallableParameter parameter : signature.parameters()) {
            if (parameters.get(parameter.ordinal() - 1) == null) {
                if (parameter.outputOnly()) {
                    parameters.set(parameter.ordinal() - 1, JavaDbPreparedSql.BoundParameter.of(null, parameter.jdbcType()));
                } else {
    ...
    protected List<String> renderedParameterLiterals() throws SQLException {
        ...
        return JavaDbPreparedSql.parse(callableSql.nativeSql()).renderLiterals(parameters);
    }
    ...
    public Clob getClob(int parameterIndex) throws SQLException {
        return JavaDbJdbcObjects.toClob(JavaDbJdbcObjects.requireTextValue(output(parameterIndex), "Clob outputs"));
    }
    ...
    public NClob getNClob(int parameterIndex) throws SQLException {
        return JavaDbJdbcObjects.toNClob(JavaDbJdbcObjects.requireTextValue(output(parameterIndex), "NClob outputs"));
    }
    ...
    public Reader getNCharacterStream(int parameterIndex) throws SQLException {
        return JavaDbJdbcObjects.toCharacterStream(
                JavaDbJdbcObjects.requireTextValue(output(parameterIndex), "NCharacterStream outputs"));
    }

    @Override
    public Reader getCharacterStream(int parameterIndex) throws SQLException {
        return JavaDbJdbcObjects.toCharacterStream(
                JavaDbJdbcObjects.requireTextValue(output(parameterIndex), "CharacterStream outputs"));
    }
    ...
    private record CallableParameter(int ordinal, String name, int columnType, int jdbcType,
        ...
        JavaDbParameterMetaData.ParameterDescriptor descriptor() {
            return new JavaDbParameterMetaData.ParameterDescriptor(
    ...
    private record CallableSignature(boolean function, CallableParameter returnParameter,
        ...
        static CallableSignature load(JavaDbConnection connection, JavaDbCallableSql callableSql) throws SQLException {
        ...
        List<JavaDbParameterMetaData.ParameterDescriptor> parameterDescriptors() {
            List<JavaDbParameterMetaData.ParameterDescriptor> descriptors = new ArrayList<>();
