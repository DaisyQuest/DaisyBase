...
final class DatabaseProtocolServer implements AutoCloseable {
    private static final String SERVER_NAME = "daisybase-server";
    ...
    private DatabaseProtocolServer(EngineApi.DatabaseEngine engine, ServerSocket serverSocket, int maxFrameBytes) {
        ...
        this.acceptThread = Thread.ofPlatform().name("daisybase-server-accept").start(this::acceptLoop);
    }
    ...
    private void acceptLoop() {
        while (!closed.get()) {
            try {
                ...
                Thread.ofVirtual().name("daisybase-server-client").start(() -> handleClient(socket));
    ...
    private String authenticatedPrincipal(String user, String password) {
        if (engine instanceof dev.daisybase.engine.EmbeddedDatabaseEngine embedded) {
